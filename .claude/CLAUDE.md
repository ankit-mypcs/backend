...
### CURRENT STATE (last updated: 18-Mar-2026)
- Status: Schema v2 deployed & LIVE on Railway — 27 models, Ancient History content pipeline live, 1,822 objects seeded
- Deployment: Gunicorn + WhiteNoise + dj-database-url, Railway (thorough-nurturing project)
- Live URL: https://backend-production-a889.up.railway.app
- Auto-deploy: GitHub push → Railway auto-build+deploy (Nixpacks builder)
- Start command (railway.json): collectstatic → migrate → seed_if_empty → gunicorn
- Env vars: ALL use os.environ.get() — django-environ fully removed from settings.py
- Database: Railway PostgreSQL (auto-provisioned, seeded via data/seed.json)
- Local DB: PostgreSQL (mypcs_db) — matches Railway data
...
- seed_if_empty: Loads data/seed.json fixture ONLY if no chapters exist (safe for every deploy)
  - Usage: python manage.py seed_if_empty (runs automatically in railway.json start command)
  - Fixture generated by gen_fixture.py (in project root, not committed)
